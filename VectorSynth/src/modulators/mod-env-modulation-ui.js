...
function getNormalizedCombo(e) {
  ...
  // For A-Z, use code for layout-independence
  let key;
  if (/^Key[A-Z]$/.test(e.code)) {
    key = e.code; // eg "KeyZ"
  } else if (/^Digit[0-9]$/.test(e.code)) {
    key = e.code; // eg "Digit1"
  } else {
    // fallback: use e.key (for F1, Escape, Arrow keys, etc)
    key =
      typeof e.key === "string"
        ? e.key.length === 1
          ? e.key.toLowerCase()
...
export function setupModEnvModulationUI({
  ...
  document.querySelectorAll(modEnvSourcesSelector).forEach((el) => {
    el.addEventListener("keydown", (e) => {
      ...
      const modRemoveShortcut = getShortcut("removeEnvMod") || "Shift+KeyP"; // or whatever your default is

      const comboStr = getNormalizedCombo(e);
      if (comboStr === modRemoveShortcut) {
        e.preventDefault();
        handleRemoveMod();
      } else if (e.key.toLowerCase() === "KeyP" && e.shiftKey) {
        // Shift+P to remove
        e.preventDefault();
        handleRemoveMod();
      }
  ...
  document
    ...
    .forEach((slider) => {
      slider.addEventListener("keydown", (e) => {
        if (e.key.toLowerCase() === "KeyP" && keyboardPickupEnv) {
          e.preventDefault();
          handlePickDrop();
        } else if (e.key.toLowerCase() === "KeyP" && e.shiftKey) {
          e.preventDefault();
