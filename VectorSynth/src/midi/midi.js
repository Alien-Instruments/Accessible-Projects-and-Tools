...
export async function setupMIDI(synth, uiContainerId = "midi-ui") {
  ...
  try {
    ...
    channelWrapper.className = "midi-select-wrapper panel";
    const headerDiv = document.createElement("div");
    headerDiv.className = "vexed-header";
    headerDiv.innerHTML = "<h1>VEXED</h1>";
...
function handleMIDIMessage(msg, synth) {
  ...
  // Let external handlers deal with note on/off, but keep pitchbend etc
  if (command === 0xe0) {
    // PITCH BEND
    const value14bit = (data2 << 7) | data1;
    synth?.setPitchBend?.(value14bit); // <-- Pass raw value
